...
class MainActivity : AppCompatActivity() {
    ...
    private fun isNetworkAvailable(): Boolean { // 인터넷 연결 가능 여부 판단
    ...
    private fun javaSocket() { // 자바 소켓 API
        output.text = ""
    ...
    private fun httpLib() { //HTTP URL Connection API
    ...
    private val receiver = object : BroadcastReceiver() { // 다운로드 완료 방송 받는 방송 수신자
    ...
    private fun downloadManager() { // URL과 다운로드 위치만 알려주면 알아서 알림도 표시하고 다운로드 수행
    ...
    private fun openDownload() { // 외부에 파일 제공
        val filePath = File(getExternalFilesDir(Environment.DIRECTORY_DOWNLOADS), "testdownload.png")
        val contentUri: Uri =
